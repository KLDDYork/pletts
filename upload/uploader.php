...
$updir = 'vacancies';		// Directory for uploads
$max_size = 30000;			// Sets maxim size allowed for the uploaded files, in kilobytes

// sets an array with the file types allowed
$allowtype = array('bmp', 'gif', 'htm', 'html', 'jpg', 'jpeg', 'mp3', 'pdf', 'png', 'rar', 'zip','doc','xls','xlsx',);
...
if (isset ($_FILES['file_up'])) {
  ...
  for($f=0; $f<count($_FILES['file_up']['name']); $f++) {
    ...
    if(strlen($fup)>1) {
      ...
      if (in_array($type, $allowtype)) {
        ...
        if ($_FILES['file_up']['size'][$f]<=$max_size*1000000) {
  ...
  echo '<br><br><a href="index.htm" title="Back to the Uploader">&lt; Back to the Uploader</a>';
}
?>